...
import { fetchSystemChannelIds } from "@/app/settings/users/actions";
import { ActionResult } from "@/types/actions";

const initialState: ActionResult = {};

export default function SystemChannelIdButton() {
  const [isLoading, setIsLoading] = React.useState(false);
  const [showDetails, setShowDetails] = React.useState(false);
  const [state, formAction] = useFormState(fetchSystemChannelIds, initialState);
    ...
    <div className="space-y-4">
      ...
      {state?.success && state?.errors && (
        <div className="rounded-lg bg-yellow-50 p-4 text-sm text-yellow-800">
          ...
          <div className="mt-1">
            処理は完了しましたが、一部のチャンネルでIDの取得に失敗しました。
          </div>
        </div>
      )}
    </div>
