...
import {
  MatchData,
  RoLIMOAStatusPayload,
  MATCH_COLLECTION,
} from "@/types/match";

/**
 * 全試合リストを match_no (または match_index) 順で取得する
 */
export async function getAllMatches(): Promise<MatchData[]> {
  const db = await getFirestore();
  const snapshot = await db.collection(MATCH_COLLECTION).get();

  const matches = snapshot.docs.map((doc) => doc.data() as MatchData);

  return matches.sort((a, b) => {
    const aNo = a.match_no ?? a.match_index ?? 0;
    const bNo = b.match_no ?? b.match_index ?? 0;
  ...
  try {
    ...
    if (matches.length === 0) {
      return {
        ok: false,
        errors:
          "Firestore matches collection is empty. Run schedule sync first.",
      };
    }

    // 現在進行中/選択された試合を特定（match_no または match_index または match_id）
    let currentMatchIndex = payload.match_no ?? payload.match_index;

    if (!currentMatchIndex && payload.match_id) {
      const found = matches.find(
        (m) => m.match_id === payload.match_id || m.id === payload.match_id,
      );

      if (found) {
        currentMatchIndex = found.match_no ?? found.match_index;
      }
    }
    ...
    if (!currentMatchIndex || currentMatchIndex < 1) {
      currentMatchIndex = matches[0]
        ? (matches[0].match_no ?? matches[0].match_index)
        : 1;
    }
    ...
    // 1. 現在の試合 (Match N) の更新
    const currentMatchDoc = matches.find(
      (m) => (m.match_no ?? m.match_index) === currentMatchIndex,
    );

    if (currentMatchDoc) {
      const isFinished =
        currentPhase === "match_finished" || payload.is_confirmed;
    ...
    const moveMatch = matches.find(
      (m) => (m.match_no ?? m.match_index) === moveMatchIndex,
    );

    if (moveMatch && !moveMatch.move_call_sent) {
      await db.collection(MATCH_COLLECTION).doc(moveMatch.id).set(
        {
          status: "moving",
          move_call_sent: true,
          updated_at: now,
    ...
    const preCallMatch = matches.find(
      (m) => (m.match_no ?? m.match_index) === preCallMatchIndex,
    );

    if (preCallMatch && !preCallMatch.pre_call_sent) {
      await db.collection(MATCH_COLLECTION).doc(preCallMatch.id).set(
        {
          status: "preparing",
          pre_call_sent: true,
          updated_at: now,
